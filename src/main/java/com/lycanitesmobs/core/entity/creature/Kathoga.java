...
public class Kathoga extends RideableCreatureEntity {
	
	PlayerControlGoal playerControlAI;

    public Kathoga(EntityType<? extends Kathoga> entityType, Level world) {
    ...
    public void specialAttack() {
        ...
        List<LivingEntity> possibleTargets = this.getCommandSenderWorld().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(distance, distance, distance), (Predicate<LivingEntity>) possibleTarget -> {
            if(!possibleTarget.isAlive()
                    || possibleTarget == Kathoga.this
                    || Kathoga.this.isEntityPassenger(possibleTarget, Kathoga.this)
                    || Kathoga.this.isAlliedTo(possibleTarget)
                    || !Kathoga.this.canAttackType(possibleTarget.getType())
                    || !Kathoga.this.canAttack(possibleTarget))
                return false;
            return true;
        });
