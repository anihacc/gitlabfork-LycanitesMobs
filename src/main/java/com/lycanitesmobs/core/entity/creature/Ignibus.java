...
public class Ignibus extends RideableCreatureEntity implements IGroupHeavy {
    protected boolean wantsToLand;
    protected boolean  isLanded;

    public Ignibus(EntityType<? extends Ignibus> entityType, Level world) {
    ...
    public void attackRanged(Entity target, float range) {
        ...
        for (int projectileX = -1; projectileX <= 1; projectileX++) {
            for (int projectileY = -1; projectileY <= 1; projectileY++) {
                Vec3 offset = this.getFacingPositionDouble(0, -6D + (0.5D * projectileY), 0, 4, this.getYRot());
