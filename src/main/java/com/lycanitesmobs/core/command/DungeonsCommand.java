...
public class DungeonsCommand {
	...
	public static int locate(final CommandContext<CommandSource> context) {
		context.getSource().sendFeedback(new TranslationTextComponent("lyc.command.dungeons.locate"), true);
		World world = context.getSource().getWorld();
		ExtendedWorld extendedWorld = ExtendedWorld.getForWorld(world);
		List<DungeonInstance> nearbyDungeons = extendedWorld.getNearbyDungeonInstances(new ChunkPos(new BlockPos(context.getSource().getPos())), ConfigDungeons.INSTANCE.dungeonDistance.get() * 2);
		if(nearbyDungeons.isEmpty()) {
			context.getSource().sendFeedback(new TranslationTextComponent("common.none"), true);
			return 0;
		}
		for(DungeonInstance dungeonInstance : nearbyDungeons) {
			context.getSource().sendFeedback(new StringTextComponent(dungeonInstance.toString()), true);
		}
		return 0;
	}
}
