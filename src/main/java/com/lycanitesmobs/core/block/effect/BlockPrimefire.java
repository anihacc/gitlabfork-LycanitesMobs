...
public class BlockPrimefire extends BlockFireBase {
	...
	//                   Constructor
	// ==================================================
	public BlockPrimefire() {
		super(Material.FIRE, LycanitesMobs.modInfo, "primefire");
        ...
        this.agingRate = 6;
        this.spreadChance = 1;
        this.removeOnTick = !ConfigBase.getConfig(this.group, "general").getBool("Features", "Enable Primefire", true);
        this.removeOnNoFireTick = ConfigBase.getConfig(this.group, "general").getBool("Features", "Remove Primefire on No Fire Tick", false);
	...
	public void burnBlockReplace(World world, BlockPos pos, int newFireAge) {
		IBlockState replaceFireBlock = this.getDefaultState().withProperty(AGE, newFireAge);
		if (world.rand.nextFloat() <= 0.25F) {
			replaceFireBlock = Blocks.FIRE.getDefaultState().withProperty(BlockFire.AGE, newFireAge);
		}
		world.setBlockState(pos, replaceFireBlock, 3);
	}
    ...
    public void onEntityCollidedWithBlock(World world, BlockPos pos, IBlockState state, Entity entity) {
		...
		if(entity instanceof EntityItem && ((EntityItem)entity).getItem() != null)
    		if(((EntityItem)entity).getItem().getItem() == ObjectManager.getItem("primefirecharge"))
    			return;

        if(ObjectManager.getEffect("penetration") != null) {
            PotionEffect effect = new PotionEffect(ObjectManager.getEffect("smouldering"), 3 * 20, 0);
    ...
    public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random random) {
        ...
        if(random.nextInt(24) == 0)
            world.playSound((double)((float)x + 0.5F), (double)((float)y + 0.5F), (double)((float)z + 0.5F), AssetManager.getSound("primefire"), SoundCategory.BLOCKS, 0.5F + random.nextFloat(), random.nextFloat() * 0.7F + 0.3F, false);
