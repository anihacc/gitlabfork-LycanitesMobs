...
public class BlockPoopCloud extends BlockBase {
	...
	public void onEntityCollision(BlockState blockState, World world, BlockPos pos, Entity entity) {
		...
		if(entity instanceof LivingEntity) {
			...
			entityLiving.addPotionEffect(new EffectInstance(StatusEffects.NAUSEA, 3 * 20, 0)); // Nausea
		}
	}
	...
	public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
		...
		if(random.nextInt(24) == 0)
			world.playSound((double)((float)x + 0.5F), (double)((float)y + 0.5F), (double)((float)z + 0.5F), ObjectManager.getSound("poopcloud"), SoundCategory.BLOCKS, 0.5F + random.nextFloat(), random.nextFloat() * 0.7F + 0.3F, false);
