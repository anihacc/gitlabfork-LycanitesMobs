...
public class CreatureInventoryScreen extends BaseContainerScreen<CreatureContainer> {
	...
	protected void initWidgets() {
		...
		String buttonText = new TranslatableComponent("gui.pet.follow").getString();
		buttonY += buttonSpacing;
		Button button = new ButtonBase(BaseCreatureEntity.PET_COMMAND_ID.FOLLOW.id, buttonX + buttonSpacing, buttonY, buttonWidth, buttonHeight, new TranslatableComponent(buttonText), this);
		...
		button = new ButtonBase(BaseCreatureEntity.PET_COMMAND_ID.WANDER.id, buttonX + buttonSpacing, buttonY, buttonWidth, buttonHeight, new TranslatableComponent(buttonText), this);
		if(!pet.isFollowing() && !pet.isSitting()) {
			button.active = false;
		}
		this.addRenderableWidget(button);
		...
		buttonY += buttonHeight + (buttonSpacing * 2);
		button = new ButtonBase(BaseCreatureEntity.PET_COMMAND_ID.SIT.id, buttonX + buttonSpacing, buttonY, buttonWidth, buttonHeight, new TranslatableComponent(buttonText), this);
		if(!pet.isFollowing() && pet.isSitting()) {
			button.active = false;
		}
		this.addRenderableWidget(button);
		...
		buttonY += buttonHeight + (buttonSpacing * 2);
		button = new ButtonBase(BaseCreatureEntity.PET_COMMAND_ID.PASSIVE.id, buttonX + buttonSpacing, buttonY, buttonWidth, buttonHeight, new TranslatableComponent(buttonText), this);
		if(pet.isPassive()) {
			button.active = false;
		}
		this.addRenderableWidget(button);
		...
		buttonY += buttonHeight + (buttonSpacing * 2);
		button = new ButtonBase(BaseCreatureEntity.PET_COMMAND_ID.DEFENSIVE.id, buttonX + buttonSpacing, buttonY, buttonWidth, buttonHeight, new TranslatableComponent(buttonText), this);
		...
		buttonY += buttonHeight + (buttonSpacing * 2);
		button = new ButtonBase(BaseCreatureEntity.PET_COMMAND_ID.ASSIST.id, buttonX + buttonSpacing, buttonY, buttonWidth, buttonHeight, new TranslatableComponent(buttonText), this);
		if(!pet.isPassive() && pet.isAssisting() && !pet.isAggressive()) {
			button.active = false;
		}
		this.addRenderableWidget(button);
		...
		buttonY += buttonHeight + (buttonSpacing * 2);
		button = new ButtonBase(BaseCreatureEntity.PET_COMMAND_ID.AGGRESSIVE.id, buttonX + buttonSpacing, buttonY, buttonWidth, buttonHeight, new TranslatableComponent(buttonText), this);
	...
	protected void drawBars(PoseStack matrixStack, int backX, int backY) {
		...
		String healthText = new TranslatableComponent("entity.health").getString() + ": " + String.format("%.0f", this.creature.getHealth()) + "/" + String.format("%.0f", this.creature.getMaxHealth());
		...
		String experienceText = new TranslatableComponent("entity.experience").getString() + ": " + this.creature.getExperience() + "/" + this.creature.creatureStats.getExperienceForNextLevel();
		...
		// Level:
		String levelText = new TranslatableComponent("entity.level").getString() + ": " + this.creature.getMobLevel();
