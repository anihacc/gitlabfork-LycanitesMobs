...
public abstract class Tab extends ButtonBase {
    ...
    public Tab(int id, int posX, int posY, ResourceLocation icon) {
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
        if(this.visible) {
            ...
            if(mc.currentScreen != null && mc.currentScreen instanceof GuiInventory) {
            	...
            	tabX += (guiInventory.width / 2) + (inventorySnooperScreen.getGUIXSize() / 2);
    			tabX -= tabWidth * (this.tabID + 1);
    			tabX -= 4;
    			
    			tabY += (guiInventory.height / 2) - (inventorySnooperScreen.getGUIYSize() / 2);
    			tabY -= ySize; 
            }
